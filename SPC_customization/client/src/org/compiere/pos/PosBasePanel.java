	...
	//implements FormPanel
{
	 ...
	 *   
	 */
	public void newOrder(String type , int bp_id)
	{	
		m_order = null;
		String paymentrule = "";
		
		//This is needed
		int DocTypeTarget_ID = 1000046;
		f_order.setM_PriceList_Version_ID(0);
		
		MBPartner mbPartner = MBPartner.get(m_ctx, bp_id);
		//set f_order info to p_pos
		f_order.setM_PriceList_Version_ID(mbPartner.getM_PriceList_ID());
		p_pos.setM_PriceList_ID(mbPartner.getM_PriceList_ID());
		paymentrule = X_C_Order.PAYMENTRULE_Cash;
		
		if(type.equals("R")){
			DocTypeTarget_ID = 1000050;
		//Credit card Order
		}else if(type.equals("Cr")){
		//Change Price list for credit card type order
			
			f_order.setM_PriceList_Version_ID(1016582);
			p_pos.setM_PriceList_ID(1016582);
			paymentrule = X_C_Order.PAYMENTRULE_CreditCard;
		}
		
		//set Business partner
		f_order.setMBpartner(mbPartner);
		m_order = PosOrderModel.createOrder(p_pos,f_order.getBPartner());
		m_order.setC_DocTypeTarget_ID(DocTypeTarget_ID);
		m_order.setPaymentRule(paymentrule);
		m_order.save();
		f_curLine.newLine();
		f_curLine.f_name.requestFocusInWindow();
		f_order.reference.setEditable(true);
		
		//enable disable when print
		f_curLine.f_name.setEditable(true);
		f_curLine.f_quantity.setEditable(true);
		f_curLine.f_delete.setEnabled(true);
		updateInfo();
	}	//	newOrder
