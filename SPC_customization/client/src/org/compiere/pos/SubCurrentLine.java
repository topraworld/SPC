...
public class SubCurrentLine extends PosSubPanel implements ActionListener, FocusListener, ListSelectionListener {
	...
	public void init() {	
		...
		m_sql = m_table.prepareTable (s_layout, s_sqlFrom, s_sqlWhere, false, "ol");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(p_posPanel.m_order.getDocAction().equalsIgnoreCase("--")){
				...
				if(entered.compareTo(new BigDecimal(0)) >=1){
					...
					if(line.getM_Product().isStocked()){
						if(results[0].getQtyAvailable().compareTo(entered) >=1)
							line.setQty(entered);
						//BigDecimal notReserved = MOrderLine.getNotReserved(p_ctx, m_M_Warehouse_ID, line.getM_Product().getM_Product_ID(), 0,0);
						else{
