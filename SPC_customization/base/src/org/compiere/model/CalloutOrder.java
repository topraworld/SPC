...
public class CalloutOrder extends CalloutEngine
{
	...
	//
	private void clearProductInfo(GridTab mTab){
		
		mTab.setValue("QTYORDERED", new BigDecimal("0"));
		mTab.setValue("QTYENTERED", new BigDecimal("0"));
		mTab.dataSave(false);
	}
			...
			&& QtyOrdered.signum() > 0)		//	no negative (returns)
		{
			...
			if (product.isStocked())
			{
				...
				if (available.signum() == 0){
					...
					this.setAllowableQty(ctx ,mTab , M_Warehouse_ID , available);
				}
				else if (available.compareTo(QtyOrdered) < 0){
					mTab.fireDataStatusEEvent ("InsufficientQtyAvailable", available.toString(), false);
					this.setAllowableQty(ctx ,mTab , M_Warehouse_ID ,available.subtract(QtyOrdered));
				}
				else
				{
					Integer C_OrderLine_ID = (Integer)mTab.getValue("C_OrderLine_ID");
					...
					if (total.compareTo(QtyOrdered) < 0)
					{
						...
						mTab.fireDataStatusEEvent ("InsufficientQtyAvailable", 
							info, false);
						this.setAllowableQty(ctx ,mTab , M_Warehouse_ID , total);
					}
				}
			}
		}
	...
	//Topra
	private void setAllowableQty(Properties ctx ,GridTab mTab , int M_Warehouse_ID , BigDecimal available){
	
		MOrderLine mOrderLine = new MOrderLine(ctx,Integer.parseInt(mTab.getValue("C_ORDERLINE_ID").toString()), mTab.getTrxInfo());
		MDocType mDocType =new MDocType(ctx,  mOrderLine.getC_Order().getC_DocTypeTarget_ID(), mTab.getTrxInfo());
		
		try{
			//Warehouse wise
			MWarehouse mWarehouse = MWarehouse.get(ctx, M_Warehouse_ID);
			//document type wise
			if(!(mWarehouse.get_ValueAsBoolean("ISALLOWMINUS_OUT") || mDocType.get_ValueAsBoolean("IS_MINUS_ALLOW"))){
				int i = available.intValue();
				
				if(i > 0){
					mTab.setValue("QTYORDERED", available);
					mTab.setValue("QTYENTERED", available);
