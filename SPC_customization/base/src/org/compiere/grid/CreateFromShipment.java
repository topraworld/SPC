		...
		*/
		...
		for (int i = 0; i < miniTable.getRowCount(); i++)
		{
			if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue()) {
				...
				//TOPRA CUSTOMIZATION : DONE BY CHATHURANGA
				//COPY INFORMATION TO ADDITIONALLY CREATED FIELDS
				MProduct mProduct = MProduct.get(Env.getCtx(), M_Product_ID);
				try{
					MProductPrice mProductPrice = MProductPrice.get(Env.getCtx(), 1000000, M_Product_ID, trxName);
					
					//SET PRICES TO MATERIAL RECIEPT
					//PHARMACY SALE
					if(mProductPrice!=null && mProductPrice.get_Value("PRICESTD_PACK")!=null)
						iol.set_CustomColumn("UNIT_RETAIL_PRICE", mProductPrice.get_Value("PRICESTD"));
					//WHOLE SALE
					mProductPrice = MProductPrice.get(Env.getCtx(), 1000001, M_Product_ID, trxName);
					if(mProductPrice!=null && mProductPrice.get_Value("PRICESTD_PACK")!=null)
						iol.set_CustomColumn("UNIT_WS_PRICE", mProductPrice.get_Value("PRICESTD"));
					//PURCHASE PRICE
					mProductPrice = MProductPrice.get(Env.getCtx(), 1000004, M_Product_ID, trxName);
					if(mProductPrice!=null && mProductPrice.get_Value("PRICESTD_PACK")!=null)
						iol.set_CustomColumn("PACK_PURCHASE_PRICE", mProductPrice.get_Value("PRICESTD_PACK"));
					
					//SET DISCOUNT
					if(mProductPrice!=null && mProduct.get_Value("DISCOUNT")!=null)
						iol.set_CustomColumn("DISCOUNT", mProduct.get_Value("DISCOUNT"));
					
					//SET TAX
					iol.set_CustomColumn("C_TAX_ID", mProduct.getC_TaxCategory_ID());
					iol.saveEx();
