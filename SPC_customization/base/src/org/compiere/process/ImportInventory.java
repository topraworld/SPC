		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (product.isInstanceAttribute())
				{
					...
					Timestamp ExpDate = TimeUtil.getDay((Timestamp)imp.get_Value("ExpDate"));
					//Auto Generated lot
					//Topra 
					if(imp.get_ValueAsBoolean("I_LOTAUTOGEN")){

						MLot mLot = new MLot(getCtx(), 0, get_TrxName());
						mLot.setAD_Org_ID(0);
						MLotCtl m_LotCtl = (MLotCtl) mas.getM_LotCtl();
						mLot.setM_Product_ID(imp.getM_Product_ID());
						mLot.setM_LotCtl_ID(mas.getM_LotCtl_ID());
						mLot.setName(m_LotCtl.getCurrentNext()+"");
						m_LotCtl.setCurrentNext(m_LotCtl.getCurrentNext() + m_LotCtl.getIncrementNo());
						m_LotCtl.save();
						mLot.save();
						imp.setLot(mLot.getName());
					}
